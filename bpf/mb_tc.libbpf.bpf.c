...
__section("classifier") int mb_tc_ingress(struct __sk_buff *skb)
{
    ...
    if (tcph->syn && !tcph->ack) {
        ...
        if (tcph->dest == in_port) {
            ...
            // bypass.
            debugf("tc <  %-6u: already rewritten", cookie);
            return TC_ACT_OK;
        }
        ...
        if (!pod) {
            ...
            debugf("tc <  %-6u: pod not found, bypassed", cookie);
            return TC_ACT_OK;
        }
        ...
        if (exclude) {
            debugf("tc <  %-6u: ignored dest port by exclude_in_ports, "
                   "ip: %pI4, "
                   "port: %d",
                   cookie, &dst_ip[3], tcph->dest);
            return TC_ACT_OK;
        }
        int include = 0;
        IS_INCLUDE_PORT(pod->include_in_ports, tcph->dest, &include);
        if (!include) {
            debugf("tc <  %-6u: ignored dest port by include_in_ports, "
                   "ip: %pI4, "
                   "port: %d",
                   cookie, &dst_ip[3], tcph->dest);
            return TC_ACT_OK;
        ...
        if (!origin) {
            ...
            debugf("tc <  %-6u: no origin", cookie);
            return TC_ACT_OK;
        }
        if (!(origin->flags & TC_ORIGIN_FLAG)) {
            // not tc origin
            debugf("tc <  %-6u: no tc origin flag", cookie);
            return TC_ACT_OK;
        }
        __u16 dst_port = tcph->dest;
        bpf_l4_csum_replace(skb, csum_off, dst_port, in_port, sizeof(dst_port));
...
__section("classifier") int mb_tc_egress(struct __sk_buff *skb)
{
    ...
    if (!(origin->flags & TC_ORIGIN_FLAG)) {
        // not tc origin
        printk("tc  > %-6u: resp origin flags %x error", cookie, origin->flags);
        return TC_ACT_OK;
    }
