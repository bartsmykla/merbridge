...
__section("cgroup/getsockopt") int mb_get_sockopt(struct bpf_sockopt *ctx)
{
    ...
    // debugf("##################SO_ORIGINAL_DST: %u", ctx->optname);
    ...
    case 2: // ipv4
        ...
        if (origin) {
            ...
            if (optval + sizeof(struct sockaddr_in) > optval_end) {
                printk("invalid getsockopt optval: optname: %d",
                ctx->optname); return 1;
            }
            ...
            *sockaddr = sa;
            ...
            // if ((void *)((struct sockaddr_in *)ctx->optval +
            //              sizeof(struct sockaddr_in)) > ctx->optval_end) {
    ...
    case 10: // ipv6
        ...
        if (origin) {
            ...
            if (optval + sizeof(*sockaddr) > optval_end) {
                printk("invalid getsockopt optval: optname: %d", ctx->optname);
                return 1;
            }
